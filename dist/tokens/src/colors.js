...
function mergeThemeObjects(base, overrides) {
    if (!overrides) {
        return base;
    }
    const result = { ...base };
    for (const [key, value] of Object.entries(overrides)) {
        ...
        if (value &&
            ...
            result[key] = mergeThemeObjects(current, value);
            continue;
        }
        result[key] = value;
    }
...
function createThemeColors(palette, overrides) {
    const base = {
        background: {
...
function createCssVarMirror(value, path = []) {
    if (typeof value === 'string') {
        return createCssVarReference(path, value);
    }
...
function flattenThemeCssVariables(value, path = [], result = {}) {
    if (typeof value === 'string') {
...
function flattenThemeTokenAliasVariables(value, path = [], result = {}) {
    if (typeof value === 'string') {
...
export function createThemeDefinition(name, label, palette, options) {
    return {
        name,
        label,
...
export const defaultThemeName = 'monokai';
function themeByAppearance(appearance) {
    return (Object.values(themes).find((theme) => theme.appearance === appearance) ??
        themes[defaultThemeName]);
}
export function resolveTheme(theme = defaultThemeName) {
    if (theme === 'dark') {
        return themeByAppearance('dark').name;
    }
    if (theme === 'light') {
        return themeByAppearance('light').name;
    }
    if (theme === 'auto') {
        if (typeof window !== 'undefined' && typeof window.matchMedia === 'function') {
            return window.matchMedia('(prefers-color-scheme: light)').matches
                ? themeByAppearance('light').name
                : themeByAppearance('dark').name;
        }
...
export function resolveThemeTokens(theme = defaultThemeName) {
    return getTheme(resolveTheme(theme));
}
export function getThemeCssVariables(theme = defaultThemeName) {
